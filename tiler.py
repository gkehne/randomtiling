...
def checkdir(board,r,c,s):
	...
	boo=True
	for i in range(tilesize):
		if 0<=r+x and r+x<b and 0<=c+y and c+y<b: #inside the box
			if board[r+x][c+y]!=0:
				boo=False
...
#Check to see if the board has 'contradictions' (unfillable spaces) in it or not.
#Possible problems for tile>2
def check(board):
	b=len(board)
	for r in range(b):
		for c in range(b):
			if board[r][c]==0 and not(checkdir(board,r,c,"u") or checkdir(board,r,c,"d") or checkdir(board,r,c,"l") or checkdir(board,r,c,"r")):
				return False
	return True
...
def findSpots(board):
	...
	for rc in range(2*b):
		if marker:
			continue
		for i in range(b):
			if rc-i<b and board[i][rc-i]==0 and neighbors(board,i,rc-i)==2:
...
#returns the number of tiled entries directly adjacent to the square at r,c in board
...
def fillBoard(board):
	...
	#print self.b
	#print self.board
	runitback=True #this fills over and over again until there are no more fillable spots
	while runitback:
		runitback=False
		for r in range(b):
			for c in range(b):
...
#This is the main section of the program. It constructs the board, while it's not full
#or stuck it iterates through finding all options on the diagonal and choosing randomly 
#between them. Finally it prints it out in progress (optional) and prints it out at the end
#
#b=dimension of square board to be randomly tiled with 1x2 tiles
#displaytype="text" for text output, ="temp" for a temporary picture, and ="filename" (no extension) for saved picture
